...
extremal regions (MSER) feature points to designate areas. %In this 
...
another.  %Another example is images that do not overlap where algorithms might still find matches adhering to the imposed geometric constraints.
...
complex geometric methods.% where simple non geometric methods might be able to provide sufficient matching accuracy with a smaller performance penalty.
...
correspondences that are distinctly matched across two images. %This method will be referred to as \emph{SIFT} in this paper. 
...
Figure~\ref{fig:comparemirror} illustrates the benefits of \emph{MM} using an example image pair from the Gallagher dataset \cite{gallagher2008}.
