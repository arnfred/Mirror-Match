...
and \emph{target image}. By similar compositions of the \emph{proposal} 
...
% TODO: include point about two of the algorithms being identical
%
...
Based on these assumptions we first prove that 
...
\appendix
\section{Proof of equivalence between \emph{Self-Match} and 
\emph{Self-Match-Ext}}
...
\section{Proof of equivalence between \emph{Mirror-Match} and 
\emph{Mirror-Match-Ext}}

In this appendix we prove the equivalence between \emph{Mirror-Match} 
and \emph{Mirror-Match-Ext}. This proof is almost identical to the 
equivalence proof between \emph{Self-Match} and \emph{Self-Match-Ext}. 
To prove this, we look at the nearest neighbor $f_{nn}$ of every query 
feature $f_{q}$ and consider the two cases of $f_{nn} \in F_{query}$ and 
$f_{nn} \in
F_{target}$. For $f_{nn} \in F_{target}$ the uniqueness ratio of 
\emph{Mirror-Match-Ext} is:
\begin{align*}
    r &= \text{r}(f_{q}, F_{proposed}, F_{baseline}) \\
        &= \text{r}(f_{q}, F_{query} \cup F_{target}, F_{baseline})\\
        &= \text{r}(f_{q}, F_{target}, F_{baseline})
...
Since $F_{proposed} = F_{target}$ for \emph{Mirror-Match} this shows that
when the nearest neighbor is found in the target image, the two 
algorithms behave identically. For $f_{nn} \in F_{query}$ we know that 
the best match for the query feature is in the query image itself. Since 
neither algorithm returns a correspondence within the same image, they 
also behave identically for this case. This proves that for any set of 
\emph{query features} \emph{Mirror-Match} returns the same matches as 
\emph{Mirror-Match-Ext}.
