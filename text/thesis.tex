...
pairwise closeness of two matches. Given a simple scene where no 
rotation and excessive affine transformation takes place, it can 
generally be assumed that the length and direction of the vector between
...
constituting the second match. They formulated an affinity function as 
follows: \TODO: change this to real function
\begin{equation*}
	S = \sum_{m_i, m_j \in T} M(m_i, m_j) = x^TMx
\end{equation*}
There are a number of occasions where this assumption no longer holds 
true. A simple example would be matches between two images of the same 
scene where one image is rotated $180\degrees$. However for a lot of 
applications like panorama stitching
where we assume that images are generally not rotated a lot in between 
each other the assumption is accurate.
...
%without geometric constraints. \emph{Isomatch} is included to 
...
\emph{Ratio}, \emph{MM}, \emph{MMC}, and \emph{Isomatch} as a function of 
...
accuracy.  Although \emph{Isomatch} exhibits a good performance on strict 
...
In terms of computational complexity, \emph{Ratio}, \emph{MM}, and \emph{Isomatch} can be 
...
\begin{tabular}{r*{4}{c}}
\hline
	Algorithm: & \emph{Ratio} & \emph{MM} & \emph{MMC} & \emph{Isomatch} 
	\\
	\noalign{\smallskip} 
	%
