...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
Here $\sigma$ is a stabilizing parameter and in the following 
experiments it is fixed to 50 after empirical optimization.  There are a 
number of occasions where the assumption that pairwise matches should 
have the same distance doesn't hold true.  A simple example would be 
matches between two images of the same scene where one image is rotated 
$180^{\circ}$.  However for a lot of applications like panorama 
stitching where we only see minor rotation and change of perspective the 
