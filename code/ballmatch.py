...
def match_speed(paths, options={}):
    ...
    indices, ks, ds = features.getFeatures(
        paths, keypoint_type, descriptor_type, shuffle_keypoints)
    ...
    def match_fun(ratio_threshold):
        matches = [(pos, s, u)
                   for pos, s, u in match_data if u < ratio_threshold]
        if len(matches) == 0:
            return [], [], []
        else:
            return zip(*matches)
...
def match_radius(paths, options={}):
    ...
    def match_fun(ratio_threshold):
        matches = [(pos, s, u, g)
                   for pos, s, u, g in match_data if u < ratio_threshold]
        if len(matches) == 0:
            return [], [], [], []
        else:
            return zip(*matches)
...
def filterNodes(bt, radius):
    ...
    # For each node, add it to akin if the node is small enough and shouldn't
    # be skipped
    for i, n in enumerate(bt.node_data):
        if (n['radius'] < radius or n['is_leaf'] == 1) and not skip[i]:
            ...
            # If parent is in pos 'i', then the two children of the binary tree
            # are at pos '2*i + 1' and '2*i+2'
            j = 2*i+1
            k = 2
